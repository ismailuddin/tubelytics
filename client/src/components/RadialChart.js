...
export default class RadialChart extends Component {
    constructor(props) {
        ...
        this.height = 450 - this.margin.bottom - this.margin.top;
        this.arcWidth = 20;
        this.chartRadius = (this.arcWidth * 5) + this.height / 5;
    ...
    arc(start) {
        return d3.arc()
            .innerRadius((d, i) => 2 + (i * this.arcWidth) + this.height / 5)
            .outerRadius((d, i) => (i * this.arcWidth) + this.arcWidth + this.height / 5)
    ...
    buildChart = () => {
        ...
        const axialAxis = this.svg.append('g')
            .attr('class', 'a axis')
            .selectAll('g')
            .data(ticks)
            .enter().append('g')
        ...
        axialAxis.append('line')
            .attr('x1', this.height / 5)
            .attr('x2', this.chartRadius);
        
        axialAxis.append('text')
            .attr('x', this.chartRadius + 15)
            .text(d => d);
        ...
        this.svg.append('g')
            ...
            .attr(
                'd', 
                d3.arc()
                .innerRadius((d, i) => (i * this.arcWidth) + this.height / 5)
                .outerRadius((d, i) => (i * this.arcWidth) + this.arcWidth + this.height / 5)
                .cornerRadius(this.cornerRadius)
    ...
    drawChart = () => {
        ...
        this.svg.append('g')
            .attr('class', 'data')
            .selectAll('path')
            .data(data.filter(d => { return d.week_of_year == this.state.weekNumber}))
            .enter()
            .append('path')
            .attr('class', 'arc')
    ...
    changeWeek = () => {
        ...
        const arcs = this.svg.selectAll('.arc')
            .data(this.data.filter(d => { return d.week_of_year == i }))
        
        this.svg.select('.week_number')
            .text('Week ' + i)
        // Remove old arcs
        arcs.exit()
            .remove()
        ...
        arcs.style('fill', (d, i) => {return this.colorScale(i)})
            .transition()
            .ease(d3.easeBounce)
            .delay((d, i) => i * 200)
            .duration(2000)
